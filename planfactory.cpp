...
PlanFactory::PlanFactory(std::shared_ptr<PlanStorage> p_s, QObject *parent) :
...
int PlanFactory::createPlan(std::shared_ptr<Cel> cel_plan)
{
    ...
    //clear data chanel
    if(cel_plan->m == 0) {
        m_data_plans->erase(cel_plan->chanel_number);
    ...
    for(uint16_t i = 0;  i < cel_plan->m; ++i) {
        ...
        if(current_sector_number != sector_number) {
            ...
            segment_ptr->appendLastCel(cel_plan);
            // else {
        ...
        if(i == lastCelIndex) {
            ...
            if (sector_status) {
                ...
                if (dataToDelete.has_value()) {
                    qDebug() << "data to delete has value";
                    ...
                    std::for_each(dataToDelete->begin(), dataToDelete->end(),
                        ...
                        if((*m_sector_plans)[plan->sector_number].is_empty()) {
                            m_sector_plans->erase(plan->sector_number);
                ...
                while(plans_to_append_it != plansToAppend.end()) {
                    ...
                    qDebug() << "Добавлен план слежения." << plans_to_append_it->seg->time_cel.time.front()
                             << plans_to_append_it->seg->time_cel.time.back() << "Канал данных"<< plans_to_append_it->chanel_number <<"физический канал"
                             << plans_to_append_it->seg->chanel_number << "сектор" << plans_to_append_it->sector_number;
                    ++plans_to_append_it;
                }
            }
            else {
...
void PlanFactory::start()
{
    //TODO:: get information about connected devices

}

void PlanFactory::stop()
{
