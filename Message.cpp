...
RecieveState::~RecieveState()
{
    if(chanel_mas != nullptr) {
         delete [] chanel_mas;
    }
}

RecieveState::RecieveState(RecieveState &&other)noexcept :  n(other.n), chanel_mas(other.chanel_mas)
{
    qDebug() << "move called";
    other.chanel_mas =nullptr;

}

RecieveState::RecieveState(const RecieveState &other) :n(other.n)
{
    qDebug() << "recive state copy consturctor called";
    if(other.chanel_mas != nullptr){
        chanel_mas = new ChanelInfo[other.n];
        for(int i = 0; i < other.n; i++) {
            chanel_mas[i]  = other.chanel_mas[i];
        }
    } else {
        chanel_mas = nullptr;
    }
...
RecieveState &RecieveState::operator =(const RecieveState & other)
{
    qDebug() << "= called";
    if(this != &other) {
        delete [] chanel_mas;
...
RecieveState &RecieveState::operator =( RecieveState && other) noexcept
{
    qDebug() << "= && caled";
    if(this != &other) {
        n = other.n;
        chanel_mas = other.chanel_mas;
        chanel_mas = nullptr;
    }
    return *this;
