...
DataChanel::DataChanel() {
    ...
    qDebug() << "DataChanel constructor called";
}

DataChanel::~DataChanel()
{
  //  qDebug() << "data chanel destructor called";
    empty = true;
    m_segments.clear();
}

DataChanel::DataChanel(const DataChanel & other)
{
  //  qDebug() << "data chanel copy called";
    for(const auto & item : other.m_segments){
       // auto ptr = std::make_shared<SegmentPlan>();
       // ptr = item;

        auto ptr = std::make_shared<SegmentPlan>(*item);
...
DataChanel &DataChanel::operator=(const DataChanel &other)
{
    qDebug() << "data chanel == caled";
    if (this != &other) {
        for(const auto & item : other.m_segments){
            auto ptr = std::make_shared<SegmentPlan>(*item);
            m_segments.push_back(ptr);
...
std::optional<std::list<std::shared_ptr<SegmentPlan> > > DataChanel::validateSegment(std::shared_ptr<SegmentPlan> new_plan)
{
    ...
    while((it != m_segments.end()) /* || (it->get()->time_cel.time.front() > end)*/) {
        ...
        if (((start >= current_start) && (start < current_end)) ||
            ...
            ((current_end <= end) && (current_end > start))) {
