...
SectorPlan::SectorPlan() {
 segment_plan  =new std::list<std::shared_ptr<SegmentPlan>>;
    qDebug() << "Sector Plan constructor called";
}

SectorPlan::SectorPlan(const SectorPlan & other) :
    empty(other.empty)
{
    qDebug() << "sector Plan copy called";
    if(segment_plan != nullptr) {
        delete segment_plan;
    }
    segment_plan = new std::list<std::shared_ptr<SegmentPlan>>;
    for(const auto & item : *other.segment_plan){
         auto ptr = std::make_shared<SegmentPlan>(* item);
        // ptr = item;
        segment_plan->push_back(ptr);
    }
}

SectorPlan &SectorPlan::operator=(const SectorPlan & other)
{
    qDebug() << "Sector plan  = called";
    if (this != &other) {
        if(segment_plan != nullptr) {
            delete segment_plan;
        }
