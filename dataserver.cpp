...
DataServer::DataServer(P2SocketHandler* p2, QObject *parent)
    : QObject{parent}, m_p2SocketHandler{p2}
{
    reset_flag = false;
        ...
        for(int i = 0; i < total_sets; ++i) {
            for(int ch = 0; ch < chanel_count; ++ch) {
                ...
                for(int sec = 1; sec <= 4; sec++) {
                    if(m_cell_storage[sec*chanel_count + ch].started()) {
                        if(m_cell_storage[sec*chanel_count + ch].size() == 0) {
                            m_cell_storage[sec*chanel_count + ch].setTime(getTime());
                        }
                        m_cell_storage[sec*chanel_count +ch].append(i_data, q_data);
                    }
                }
            }
        }
...
void DataServer::onAcceptCel(int data_chanel, int real_data_chanel, int sector,const QDateTime& start_time,int ka_number, int az, int angle)
{
        ...
        az_[0]= az;
        az_[1] = angle;
    if(!m_cell_storage[sector *  chanel_count + real_data_chanel].started()) {
        m_cell_storage[sector *  chanel_count + real_data_chanel].set(data_chanel,0,ka_number,start_time,az_);
    }
    else {
        if(m_cell_storage[sector *  chanel_count + real_data_chanel].size() != 0) {
            m_cell_storage[sector *  chanel_count + real_data_chanel].sendAll();
        }
        m_cell_storage[sector *  chanel_count + real_data_chanel].set(data_chanel,0,ka_number,start_time,az_);
...
void DataServer::onStop(int data_chanel, int real_data_chanel, int sector)
{
    ...
    Q_ASSERT(m_cell_storage[sector *  chanel_count + real_data_chanel].started());
    if(m_cell_storage[sector *  chanel_count + real_data_chanel].size() > 0) {
        m_cell_storage[sector *  chanel_count + real_data_chanel].sendAll();
    }
    m_cell_storage[sector *  chanel_count + real_data_chanel].finish();
}
...
bool DataServer::checkHeader(const packet_header *packet_header)
{
    ...
    if (packet_header->counter != (++counter)) {
        qDebug() << "counter" << counter << packet_header->counter;

        counter = packet_header->counter;
    }
...
void DataServer::connect_server()
{
    ...
    connect(tcp_socket, &QTcpSocket::stateChanged, this, [](int state)
            {
                ...
                {
                    qInfo() << "modbus TcpSocket connected";
                }
            });
    ...
    for(int i = 6; i <= 30; ++i) {
       ...
       // m_cell_storage[i].moveToThread(storage_thread);
        connect(&m_cell_storage[i],&CellStorage::readyToWriteData, m_p2SocketHandler,&P2SocketHandler::sendData);
    }
...
{
    qDebug() << "Полная остановка передачи в П2";
...
{
    qDebug() << "Остановка передачи данных в П2 для канала данных" << data_chanel << ", физаческого канла"<< real_data_chanel << "сектора"<< sector;
    Q_ASSERT(m_cell_storage[sector *  chanel_count + real_data_chanel].started());
    if(m_cell_storage[sector *  chanel_count + real_data_chanel].size() > 0) {
        m_cell_storage[sector *  chanel_count + real_data_chanel].sendAll();
    }
...
{
    if(m_cell_storage[sector *  chanel_count + real_chanel_number].started()) {
        qDebug() << "Остановка передачи данных в П2 для канала данных" << data_chanel_number << ", физаческого канла"<< real_chanel_number<< "сектора"<< sector;
        if(m_cell_storage[sector *  chanel_count + real_chanel_number].size() > 0){
            m_cell_storage[sector *  chanel_count + real_chanel_number].sendAll();
        }
        m_cell_storage[sector *  chanel_count + real_chanel_number].finish();
    }
}
